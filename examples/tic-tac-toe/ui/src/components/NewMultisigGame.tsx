	...
	return (
		<>
			...
			<Em>
				...
				fast path (single-owner) transactions: One from the player's address to authorize the move,
				and one from the multi-sig, signed by the player's address, to make the move.
			</Em>
		</>
	);
}
...
function parsePublicKey(key?: string): PublicKey | null {
	...
	try {
		...
		console.error('Failed to get public key from sui bytes', e);
		return null;
	}
}
